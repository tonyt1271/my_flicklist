...
# DONE    Notice that we've already created a 'login' link in the upper-right corner of the page that'll connect to it.
...
def register():
    if request.method == 'POST':
        ...
        if not is_email(email):
            flash('zoiks! "' + email + '" does not seem like an email address','error')
...
def add_movie():
    ...
    for t_movie in watchlist:
        if new_movie_name == t_movie.name: 
            error = "{0}' is already in your Watchlist, or you've already watched it".format(new_movie_name)
            return redirect("/?error=" + error)
...
# DONE       It should contain 'register' and 'login'.
...
def require_login():
    ...
    print(session)
    if request.endpoint not in allowed_routes and not 'user' in session:
